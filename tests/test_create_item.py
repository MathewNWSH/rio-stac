...
def s2_safe_dir(tmp_path):
    ...
    if os.path.exists(fixture_path):
        shutil.copy(fixture_path, granule_dir / "T31VEE_20251128T111431_B02.jp2")
...
def test_s2_l1c_builder(s2_safe_dir):
    ...
    assert any(k.endswith("B02") for k in assets)
...
def test_general_post_processing(s2_safe_dir):
    ...
    for k in thumb:
        if k.startswith("proj:") and k != "proj:code":
            assert False, f"Thumbnail has forbidden field {k}"
    ...
    # Find a band
    band_key = next(k for k in assets if "B02" in k)
